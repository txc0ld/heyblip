...
struct FriendsListView: View {
    ...
    private func acceptFriendRequest(_ item: FriendListItem) {
        ...
        guard let friend = try? context.fetch(FetchDescriptor<Friend>()).first(where: { $0.id == item.id }) else { return }
        Task {
    ...
    private func loadFriends() {
        let context = ModelContext(modelContext.container)
        guard let allFriends = try? context.fetch(FetchDescriptor<Friend>()).sorted(by: { $0.addedAt > $1.addedAt }) else { return }
